...
function currentRussianCandidates(){
  ...
  let state;try{state=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')}catch(e){return []}
  let words=Array.isArray(state.words)?state.words:[];
  let matches=words.filter(w=>normDe(w?.de)===normDe(prompt));
  if(matches.length>1){
  ...
  return unique(matches.flatMap(w=>[w.ru,...(w.altRu||[])]));
}
...
function scorePair(source,target){
  ...
  const sim=Math.max(charSim,bestToken);
  const len=t.replace(/\s/g,'').length;
  const dist=levenshtein(s,t);
  let accept=false;
  if(targetWords.length===1){
    if(len<=3)accept=sim===1;
    else if(len<=5)accept=sim>=0.80||dist<=1;
    else if(len<=8)accept=sim>=0.72||dist<=2;
    else accept=sim>=0.70||dist<=3;
  ...
  if(sourceWords.length>=targetWords.length){
    let tokenSum=0,matched=0;
    for(const tw of targetWords){let best=0;for(const sw of sourceWords)best=Math.max(best,similarity(sw,tw));tokenSum+=best;if(best>=0.7)matched++;}
    const avg=tokenSum/targetWords.length;
    if(avg>=0.82&&matched===targetWords.length)accept=true;
    charSim=Math.max(charSim,avg);
  }
  if(charSim>=0.78)accept=true;
  return {score:charSim,accept};
}
function bestMatch(raw){
  ...
  for(const source of sources){for(const target of candidates){
    ...
    const r=scorePair(source,target);
...
function rememberResults(e){
  const prompt=($('#promptText')?.textContent||'').trim();
  if(prompt!==latestPrompt){latestAlternatives=[];latestPrompt=prompt;}
  const arr=[];
  try{
    const combined=[];
    for(let i=0;i<e.results.length;i++){
      const res=e.results[i];if(res?.[0]?.transcript)combined.push(res[0].transcript);
      for(let j=0;j<Math.min(5,res.length);j++)if(res[j]?.transcript)arr.push(res[j].transcript);
    }
    if(combined.length)arr.unshift(combined.join(' '));
  }catch(err){}
...
function installRecognitionWrapper(){
  ...
  if(!Native||Native.__rvtToleranceWrapped)return;
  function Wrapped(){
    const r=new Native();
    try{r.addEventListener('result',rememberResults);}catch(e){}
    try{
      const phrases=currentRussianCandidates();
      const Phrase=window.SpeechRecognitionPhrase;
      if(Phrase&&'phrases'in r&&phrases.length)r.phrases=phrases.slice(0,12).filter(x=>/[а-яё]/i.test(x)).map(x=>new Phrase(x,4.5));
    }catch(e){}
    return r;
  }
  try{Object.setPrototypeOf(Wrapped,Native);Wrapped.prototype=Native.prototype;}catch(e){}
  Wrapped.__rvtToleranceWrapped=true;
...
function installSubmissionCorrection(){
  document.addEventListener('click',e=>{
    ...
    const match=bestMatch(raw);if(!match)return;
    input.dataset.voiceRaw=raw;input.value=match.target;
  },true);
}
