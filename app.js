...
const REVIEW_DAYS=[7,30,90,180,365];
...
function renderProgress(){const total=state.words.length,mastered=state.words.filter(w=>{const p=pFor(w.id);return p.level>=5&&!p.active}).length,d=today();$('#statTotal').textContent=total;$('#statMastered').textContent=mastered;$('#statToday').textContent=d.answers;$('#statAccuracy').textContent=d.answers?Math.round(d.correct/d.answers*100)+' %':'–';$('#statStreak').textContent=state.streak.current||0;$('#statBestStreak').textContent=state.streak.best||0;
 const groups=[['Neu',p=>!p.seen&&!p.postponed],['Lernen',p=>p.active],['Hinten',p=>p.postponed],['Sicher',p=>p.level>=5&&!p.active]];$('#levelBars').innerHTML=groups.map(([n,fn])=>{const c=state.words.filter(w=>fn(pFor(w.id))).length,pc=total?c/total*100:0;return `<div class="level-bar-row"><span>${n}</span><div class="bar-track"><div class="bar-fill" style="width:${pc}%"></div></div><strong>${c}</strong></div>`}).join('');
...
function renderSettings(){const s=state.settings,input=$('#settingNewLimit');$('#settingDirection').value=s.direction;if(input){input.value=s.newLimit;input.min='5';input.max='30';const label=input.closest('label');if(label&&label.firstChild&&label.firstChild.nodeType===3)label.firstChild.textContent='Gleichzeitig aktive Vokabeln\n            ';}$('#settingTyping').checked=s.typing;$('#settingInfinitiveFirst').checked=s.infinitiveFirst;$('#settingAutoplay').checked=s.autoplay;$('#settingStrict').checked=s.strict;document.body.classList.toggle('dark',s.dark);$('#themeToggle').textContent=s.dark?'☀':'☾';}
...
function saveWordFromForm(ev){ev.preventDefault();const id=$('#editId').value||`u${Date.now()}`,word={id,ru:$('#editRu').value.trim(),de:$('#editDe').value.trim(),altDe:splitSemi($('#editAltDe').value),altRu:splitSemi($('#editAltRu').value),type:$('#editType').value,topic:$('#editTopic').value.trim(),note:$('#editNote').value.trim(),forms:$('#editForms').value.trim()};if(!word.ru||!word.de)return;const i=state.words.findIndex(w=>w.id===id);if(i>=0)state.words[i]=word;else state.words.push(word);save();$('#wordDialog').close();renderWords();updateSummary();ensureActiveWindow();selectNext();toast(i>=0?'Vokabel gespeichert':'Vokabel hinzugefügt');}
function removeWordById(id){const w=state.words.find(x=>x.id===id);if(!w)return false;state.words=state.words.filter(x=>x.id!==id);delete state.progress[id];state.postponedIds=state.postponedIds.filter(x=>x!==id);if(id.startsWith('v')){state.deletedDefaultIds=state.deletedDefaultIds||[];if(!state.deletedDefaultIds.includes(id))state.deletedDefaultIds.push(id)}return true;}
function deleteCurrentEdit(){const id=$('#editId').value;if(!id)return;const w=state.words.find(x=>x.id===id);if(!confirm(`„${w?.ru||''}“ wirklich löschen?`))return;if(!removeWordById(id))return;save();$('#wordDialog').close();renderWords();ensureActiveWindow();selectNext();toast('Vokabel gelöscht');}
function discardCurrentWord(){if(!current)return;const w=current;if(!removeWordById(w.id))return;save();current=null;ensureActiveWindow();selectNext();renderProgress();toast(`„${w.ru}“ verworfen`);}
function postponeCurrentWord(){if(!current)return;const w=current,p=pFor(w.id);p.active=false;p.postponed=true;p.nextTurn=state.sessionTurn;if(!state.postponedIds.includes(w.id))state.postponedIds.push(w.id);save();current=null;ensureActiveWindow();selectNext();renderProgress();toast(`„${w.ru}“ hinten angestellt`);}
