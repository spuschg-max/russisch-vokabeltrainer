...
let desiredAuto=true,sequence=0,installed=false;
...
function pauseAuto(){const cb=$('#conjAutoMic');if(cb)desiredAuto=cb.checked;setAuto(false);if(micOn())try{$('#conjMic').click()}catch(e){}}
...
async function announceTask(){
  ...
  $('#conjStatus').textContent='Aufgabe wird vorgelesen …';
  await speak([verb,person].filter(Boolean).join('. '),.73);
  if(my!==sequence||!active()||!resultHidden())return;
  $('#conjStatus').textContent=desiredAuto?'Ich höre …':'';
  if(desiredAuto){setAuto(true);setTimeout(()=>{if(active()&&resultHidden()&&!micOn())try{$('#conjMic').click()}catch(e){}},180)}
}
...
async function finishCorrect(){
  ...
  const my=++sequence;pauseAuto();box.dataset.rvtHeld='1';box.classList.add('hidden','rvt-conj-hold');
  await waitUntilSpeechEnds();
  if(my!==sequence||!active())return;
  box.classList.remove('rvt-conj-hold');delete box.dataset.rvtHeld;
  // hidden bleibt bis zur neuen Karte: dadurch kann der alte 1,1-s-Timer nicht mehr selbst weiterschalten.
  const next=$('#conjNext');if(next)next.click();
}
function prepareNext(){pauseAuto();++sequence;setTimeout(announceTask,120)}
...
function install(){
  ...
  document.addEventListener('click',e=>{
    if(e.target?.closest?.('#conjugationTab')){pauseAuto();++sequence;setTimeout(announceTask,160);return}
  ...
  const person=$('#conjPerson');if(person)new MutationObserver(()=>{
    if(active()&&resultHidden())setTimeout(()=>{if(!speechBusy())announceTask()},80);
